...
Note: the setup is not rootless, because we need the rights to set the `CAP_SYS_ADMIN` capability.
...
Linux-only cli tests are only run in Docker.

If you’re on Linux, you can only run `just cli-tests-docker` to avoid running twice the cli tests that are not linux-only.

If you’re not Linux, using `just cli-tests` ensures cli tests work on both your OS and Linux.

### Regenerating cli tests assertions
...
- `{platform}` can be:
    ...
    - `unix`, `windows`: for tests run on these specific OSes or family of OSes
    - `macos`, `*bsd`: we don’t have specific cli tests for them yet, but we could since we have some of these in the CI
